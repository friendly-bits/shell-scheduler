...
test_core_03() {
	...
	print_test_header 3 "Failure status propagation to done_handler" "${jobs}"

	SCHED_FAIL_MSG_CB=echo \
	SCHED_FINALIZE_CB=finalize_handler \
	JOB_DONE_CB=test_core_03_done_handler \
...
test_core_04() {
	...
	print_test_header 4 "DO_JOB_CB return statuses" "${jobs}"

	SCHED_FAIL_MSG_CB=echo \
	SCHED_FINALIZE_CB=finalize_handler \
	JOB_DONE_CB=test_core_04_done_handler \
	SCHED_MAX_JOBS=3 \
	SCHED_TIMEOUT_S=20 \
	SCHED_IDLE_TIMEOUT_S=10 \
	DO_JOB_CB=test_core_04_do_job \
		schedule_jobs "${jobs}" &
...
test_core_05() {
	...
	print_test_header 5 "noglob state preserved in callbacks" \
	...
	for mode in glob noglob; do
		...
		SCHED_FAIL_MSG_CB=echo \
		SCHED_FINALIZE_CB=test_core_05_finalize_handler \
		JOB_DONE_CB=test_core_05_done_handler \
		DO_JOB_CB=test_core_05_do_job \
		SCHED_MAX_JOBS=2 \
		SCHED_TIMEOUT_S=3 \
		SCHED_IDLE_TIMEOUT_S=2 \
