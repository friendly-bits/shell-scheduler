...
Note: `job_termination` covers the modular job termination feature (`JOB_TERM_CB`) with variant-shared tests driving the selected variant's default mechanism, plus the shared infrastructure the `*_full` / `*_mini` files reuse. The full-variant `job_termination_full` category covers the three job termination mechanisms built into `scheduler.sh`: cgroup, `/proc` children-walk, and `/proc` PPID-walk. Each mechanism's tests are gated on that mechanism being usable in the current environment, and report SKIP otherwise (counted separately in the summary):
