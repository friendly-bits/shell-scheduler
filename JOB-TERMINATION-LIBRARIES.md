...
This document explains, in depth, the two callback implementations bundled with the project. For a short overview and the one-line "how do I switch it on" usage, see [REFERENCE.md](REFERENCE.md#job-termination-helper-libraries); if you are just getting started, read the [README](README.md) first.
...
- [`scheduler-job-term-proc.sh` - the /proc library](#scheduler-job-term-procsh---the-proc-library)
  - [Mechanism](#mechanism)
...
Both libraries implement the same callback contract and terminate a job's whole process tree, but the mechanism is very different:

| Aspect                                       | `/proc`-based library                       | `cgroup`-based library                                                 |
| -------------------------------------------- | ------------------------------------------- | ---------------------------------------------------------------------- |
| Requirements                                 | `/proc` and `awk` only; no root, no cgroups | cgroup v2 + `cgroup.kill` (kernel >= 5.14) + writable cgroup           |
| Finds processes reparented to init (orphans) | No                                          | Yes                                                                    |
| Process kill verification                    | None                                        | Kernel-verified                                                        |
| `<running_pids>` reported at cleanup         | May list PIDs whose trees are already dead  | Only lists PIDs under fault conditions when process termination failed |
| Side effects on the tree                     | Brief `SIGSTOP` before the kill             | None                                                                   |
| Performs per-run setup and teardown          | No                                          | Yes                                                                    |

Rule of thumb: prefer the `cgroup`-based termination mechanism wherever its requirements are met (see [When the scheduler can manage cgroups](#when-the-scheduler-can-manage-cgroups)); fall back to the `/proc`-based mechanism otherwise. The mechanism can be selected automatically at runtime - see [Selecting job termination mechanism at runtime](#selecting-job-termination-mechanism-at-runtime).
...
The below table shows the **action** taken by the two helper libraries in response to each subcommand.

| Subcommand | `/proc`-based library          | `cgroup`-based library                               |
| ---------- | ------------------------------ | ---------------------------------------------------- |
...
## `scheduler-job-term-proc.sh` - the /proc library

Kills each job's process tree by reconstructing it from the kernel's `/proc` data, with no dependency on cgroups or elevated privileges. Only `/proc` and `awk` are required, which makes it the portable fallback - it works on essentially any Linux, including environments where cgroup delegation is unavailable.
...
1. **Discover.** The tree is walked breadth-first starting from the seeds. For each process in the current frontier, the library reads every `/proc/<pid>/task/<tid>/children` file - iterating over *all* of the process's threads (`task/*`), so that children forked by a non-leader thread are still found. The `children` files are parsed with `awk`. Newly seen PIDs form the next frontier; a "seen" set prevents revisiting.
...
- `init`, `setup`, `cleanup` - no-ops (return success). This library needs no per-run or per-job setup and holds no state between calls.
...
## `scheduler-job-term-cgroup.sh` - the cgroup v2 library
...
- `init` - locate the cgroup v2 mount (from `/proc/mounts`), choose and create the base cgroup (see [`SCHED_CGROUP_BASE`](#sched_cgroup_base) for how the location is picked), confirm `cgroup.kill` exists (kernel >= 5.14), and validate the whole mechanism by moving a throwaway probe process into a child cgroup. A stale `sched_<pid>` left by a previous run that reused this PID is cleared first. Any failure here fails the run upfront (see [Requirements](#requirements)).
...
- Write access to a cgroup directory, so the scheduler can create the per-run and per-job cgroups and move processes into them. See [When the scheduler can manage cgroups](#when-the-scheduler-can-manage-cgroups) for when this holds.

### When the scheduler can manage cgroups

The mechanism is usable in each of the situations below, none of which needs any pre-configuration. If none of them fits your environment - or arranging one is inconvenient - use `scheduler-job-term-proc.sh` instead (or let `cgroup_cleanup_supported` pick it automatically, see [Selecting job termination mechanism at runtime](#selecting-job-termination-mechanism-at-runtime)): that library needs only `/proc` and `awk`.
...
- **As root** (a system crontab, or busybox `crond` on OpenWrt): use `schedule_jobs` directly - nothing extra is required.
...
Your application can choose the mechanism automatically based on what the current environment supports, using `cgroup_cleanup_supported` as the test:
